...
\item $\tily_3=\unit[100]{kg}$ plastic (\unit[40]{\%} of which 
      can be recycled)
\item $\tily_4=\unit[80]{kg}$ rubber  (\unit[25]{\%} of which can be recycled)
\item $\tily_5=\unit[36]{kg}$ lead (three starter batteries \`a \unit[12]{kg})
\item $\tily_6=\unit[15\,000]{l}$ gasoline (\unit[250\,000]{km} at
...
etc is comparable to that of the ICE vehicles, including the (smaller,
\unit[8]{kg} instead of \unit[12]{kg})
lead starter batteries.\\[1ex] 
\pause \item Additionally, a BEV has Lithium driving
...
\maindm{
\vec{\tily}=(\unit[800]{kg},\ \unit[60]{kg},\ \unit[60]{kg},\
\unit[60]{kg},\ \unit[24]{kg},\ \unit[600]{kg},\ \unit[50\,000]{kWh})\tr.
}
\vspace{1em}

\pause \item The changed inventory items items lead to a
 new \COii\ emission factors row vector
\maindm{
\vec{C}'\sub{\COii}=(4,\ 30,\ 2,\ 2,\ 20,\ 20,\ \unit[0.38]{kg/kWh}).
}
\vspace{1em}

\bi
\item The Li driving batteries are expensive to produce and there is
  much controversy in estimating their overall emission factor
  $C_{15}$\\[1ex]
\pause \item The energy emission factor is based on the latest (2023)
  German energy mix emitting \unit[380]{g} \COii\ per kWh of electrical
  energy at the socket %450 g/kWh as of 2019
\ei
...
\unit[20]{kWh} * \unit[0.38]{kg/kWh}=\unit[7.6]{kg}. The ICV vehicle
emits directly and indirectly
\unit[6]{l} * \unit[2.7]{kg/l}=\unit[16.2]{kg}. So, the BEV \COii
emissions per kilometer are somewhat less than half that of the ICE
(internal combustion engine, gasoline) vehicle. 
\\[1ex]
However, the BEV production emissions are significantly
higher. Furthermore, less than ideal efficiencies when
charging/discharging (total efficiency $\approx \unit[80]{\%}$) have
not been considered.}\\[1em] 
...
\pause \itemAnswer \colAnswer{We saw that the \emph{driving} emissions $C'$
  ...
  $C^0$ due to 
  production/disposal/recycling. So, we have a positive break-even
  kilometrage obtained by solving for $x$ the
...
\itemAsk \colAsk{Give the two most important external factors influencing the total LCA
...
\item[(i)] \colAnswer{The energy mix of the used electricity
  ...
  indulgences [ger: \emph{Ablassbriefe}] /ordering ``green'' electricity)}
\pause \item[(ii)] \colAnswer{The production and disposal/recycling emissions of the
  ...
  (to research this is even more tricky).}
\ei
}
...
to install your PV and
  determine the average power (in Germany, this so-called
  \emph{availability} is about \unit[12]{\%} of the
  installed power $P\sub{max}$) and calculate the total electric
  energy delivered, e.g., $W\sub{el}=0.12\, \tau P\sub{max} $ }
\pause \item[(iii)] \colAnswer{Get the production and recycling 
emissions $C$ of your PV with an indicated power $P\sub{max}$ including 
