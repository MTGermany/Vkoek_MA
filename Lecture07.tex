...
\pause \item The factors come in four categories, three of which
contain exogeneous variables:
\bi
\item \bfdef{alternative-specific constants (ACs)}
  play the role of constants in regression models,
\item \bfdef{characteristics} are attributes of the alternatives (and
  may also depend on the decision maker)
\item \bfdef{socioeconomic factors} are attributes of the decision
  makers alone,
...
$V_{ni}=\beta^*_{02}\delta_{2i}+ ... + \beta^*_{0I}\delta_{Ii}$ where 
$\beta^*_{0i}=\beta_{0i}-\beta_{01}$,
particularly $\beta^*_{0I}=-\beta_{01}$}}
\ei
...
\item \bfdef{alternative-specific:} \
...
% !!! I use the \sum_j notation only if the reference has another
% expression Otherwise (e.g. interacting S with C, a direct
...
\pause \item let it \bfdef{interact} with a characteristic $m'$, e.g.,
  $X_{mni}=S_{mn}C_{m'ni}$ and formulate it in a generic 
($V_{ni}=...+\beta_mS_{mn}C_{m'ni}$) or alternativ-specific way,
...
PT (Alt. $i=3$) over other modes more than men?}\\

\pause \colAnswer{\small{
Assume $S_{mn}=g_n=1$ for woman and =0 for men. 
...
\pause \colAnswer{\small{Let the income $I_n$ interact with the
    ...
    expected to be negative. Still better is an alternative-specific
    formulation: $+\beta_{Ii}I_nT_{ni}$ }}
...
$
\begin{array}{l}
V_1 =  \beta_0 + \beta_2 K_1 + \beta_{31} T_1,\\
V_2 =  \beta_1 + \beta_2 K_2 + \beta_{32} T_2,\\
...
%##############################################################
\frame{\frametitle{Example: RP Survey in the audience, April 2018%old}
%##############################################################
