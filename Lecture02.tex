%#############################################
% !!! Bereits in Bachelor zwei Lectures 02/03 gebracht. 
% !!! Erwaehne/modifiziere!
%#############################################
...
\pause \item \textbf{Consequences of superfluous factors:} 
...
\pause \item Black bullets: data (right graphics: twice mirrored)
...
\pause \item Air drag: proportional to speed squared, $\tilde{x}_1^2$, and independent
  from mass
...
\placebox{0.5}{0.15}{
 \parbox{0.9\textwidth}{
  ...
  would linearize the model but the fluctuations are not i.i.d (see
  statistical 
  specification below)}}
...
\item The expectation value of the residual deviation should be
  $E(\epsilon)=0$.
\pause\item \textbf{Consequences:} \blue{None}: The Ordinary Least
...
but is no longer efficient} (a medium error).
...
\pause \item \textbf{Consequences:} \bforange{medium:}
(OLS estimator not efficient; underestimation of estimation errors;
...
\pause \item \textbf{Consequences:} a violation has \bfgreen{mild}  consequences: OLS remains
...
\item This means, the data should overdetermine the model which is the
  basis for fitting.
\pause \item \textbf{Consequence of a violation:} If $n=J+1$, the data determine the
model exactly, i.e., 
...
analysis will return a ``0/0 error''

\pause \item \textbf{Consequence of satisfying the requirement
  borderline:} If there are only a few more data points than
parameters, i.e., only a few \bfdef{degrees of freedom}, the
...
combination of other exogenous variables. Otherwise, the data matrix
is \emph{singular} 
...
\bfdef{Gau\3-Markov assumptions}}
...
x_{ik}-\bar{x}_k)
\edm
has the full rank $J+1$, i.e., $\text{det}\,\m{S}\neq 0$
\pause\item For $n<J+1$, this is not satisfied trivially
\ei
...
\edm 
satisfying the Gau\3-Markow specifications (the Gaussian distribution
of the $\epsilon_i$ is not required, here)
