...
(n-J-1)\hatsig^2
 ...
 - (\m{X}\vecbeta)\tr\vec{y}-\vec{y}\tr(\m{X}\vecbeta)
 + \vec{y}\tr\vec{y}
\end{align*}
...
Finally, we replace the observed endogeneous data vector $\vec{y}$ by
the model $\vec{y}=\m{X}\vecbeta+\veceps$ {\scriptsize \red{Notice: the
    true and, 
  according to the Gau\3-Markow assumptions, immutable 
  parameter vector $\vecbeta$ is used here!}}:
...
\end{align*}
After doing the simplification, we realize that the second and third
term are each equal to zero, so we have the final result
\bdm
(n-J-1)\hatsig^2
=\veceps\tr (\m{1}-\m{X}(\m{X}\tr\m{X})^{-1}\m{X}\tr)\veceps
\edm
With the Gau\3-Markow-assumptions, this is proportional to a sum of
$(n-J-1)$ squared Gaussians, i.e., a $\chi^2(n-J-1)$ distributed
random variable
}
...
approximate estimation errors are student-t distributed (a Gaussian in
the numerator and the square root of a $\chi^2$ distributed random
