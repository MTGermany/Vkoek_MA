...
%##############################################################
\frame{\frametitle{MNL: The \red{Red-Bus}-\blue{Blue-Bus} Problem}
%##############################################################
...
%##############################################################
\frame{\frametitle{100\% correlated random utilities: Problem solved!}
%##############################################################
...
\pause
\vspace{1em}
\red{$\Rightarrow$ complicated and non-intuitive!}
...
\pause
\parbox{0.30\textwidth}{\footnotesize{(calibrate first
    $e^{\tilde{V}_{lm}/\lambda_l}$, then determine $\lambda_l$ with
    fixed $I_l$ in the outer MNL calibration)}}
\ei
...
\pause \colAnswer{Because for these assumptions $P_l$ has
    the normal  MNL form}
...
that $\lambda_lI_l=\tilde{V}_{lm^*_l}$ for $\lambda_l\to 0$.
\pause \colAnswer{All contributions of the sum inside the log are
  exponentials and thus positive.\\[-0.7ex]Furthermore, the $\ln$ function is
...
\pause \colAnswer{Independent because $P_{lm}=P_l P_{m|l}$, MNL for
...
% transferred to Bachelor's course
% see German script, Sec. 4.15
