...
\frac{x_{mnj}}{P_{ni}}\ablpart{P_{ni}}{x_{mnj}}
=-\beta_mx_{mnj}P_{nj}
}
\ei
}
...
$x_{mnj}$: 
\bdma
\ablpart{P_{ni}}{x_{mnj}}
 &=& \frac{e^{V_{ni}}}{\sum_k e^{V_{nk}}}\ablpart{V_{ni}}{x_{mnj}}
  - \frac{e^{V_{ni}}}{\left(\sum_k e^{V_{nk}}\right)^2}
    \ablpart{}{x_{mnj}}\left(\sum_l e^{V_{nl}}\right) \\
\visible<3->{ &=& P_{ni}\ablpart{V_{ni}}{x_{mnj}}
  - \sum_l \frac{e^{V_{ni}}e^{V_{nl}} }{\left(\sum_k e^{V_{nk}}\right)^2}
    \ablpart{V_{nl}}{x_{mnj}} \\}
\visible<4->{ &=& P_{ni}\ablpart{V_{ni}}{x_{mnj}}
  - \sum_l P_{ni} P_{nl} \ablpart{V_{nl}}{x_{mnj}}}
\edma
\visible<5->{where
\bdm
\ablpart{V_{ni}}{x_{mnj}}=\beta_m\delta_{ij}, \quad
\ablpart{V_{nl}}{x_{mnj}}=\beta_m\delta_{lj}}
\edm
\visible<6->{Hence
\bdm
\ablpart{P_{ni}}{x_{mnj}}=\beta_mP_{ni}\left(\delta_{ij}-P_{nj}\right),\quad
\epsilon_{nij}\sup{(mic,m)}=\frac{x_{mnj}}{P_{ni}}\ablpart{P_{ni}}{x_{mnj}}
=\beta_mx_{mnj}\left(\delta_{ij}-P_{nj}\right)
\edm
}
\visible<7->{$j=i$: 
$\epsilon_{nii}=\beta_mx_{mni}\left(1-P_{ni}\right)$, \quad $j\neq i$:
$\epsilon_{nij}=-\beta_mx_{mnj}P_{nj}$}
}}
\ei
}
...
\visible<3->{ &=-\sum_{i\neq j} P_{ni}\beta_mx_{mnj}P_{nj}
 + P_{ni}\beta_mx_{mni}\left(1-P_{ni}\right)\\}
\visible<4->{ &=\beta_m \left(-\sum_i P_{ni} P_{nj} x_{mnj}+P_{ni}x_{mni}\right)=0}
\end{align*}
...
$P_{ni}\epsilon\sup{(m)}_{nij}=x_{mnj}\ablpart{P_{ni}}{x_{mnj}}$ and
    $x_{mnj}$ is constant for the summation over $i$)}
}}
...
\quad X_{mj}=\sum_{n=1}^N x_{mnj}, \quad
N_i=\sum_{n=1}^NP_{ni}
}
...
\item[(i)] Same absolute changes for all persons,
  $\diff{x_{mnj}}=\diff{X_{mj}}/N$:
\bdm
\epsilon_{ij}\sup{(mac,abs,m)}
= \frac{X_{mj}}{N_i}\ 
\frac{1}{N}\sum_n\frac{P_{ni}}{x_{mnj}}\epsilon_{nij}\sup{(mic,m)}
\edm
\vspace{-1ex}

\pause \item[(ii)] Same relatives changes for all,
$\diff{x_{mnj}}/x_{mnj}=\diff{X_{mj}}/X_{mj}$:
\bdm
